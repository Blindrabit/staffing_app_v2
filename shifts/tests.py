...
class ShiftsTests(TestCase):
    ...
    def test_shifts_create(self):
        ...
        self.assertEqual(f'{self.shifts.start_time}',
                         '2020-09-30 08:00:00+00:00')
        self.assertEqual(f'{self.shifts.end_time}',
                         '2020-09-30 17:00:00+00:00')

    def test_shifts_list_view(self):
        self.client.login(email='test_username@example.com',
                          password='da_password')
    ...
    def test_shifts_create_view_logged_out(self):
        response = self.client.get(reverse('createshift'))
        self.assertRedirects(response, '/accounts/login/?next=/shifts/create/',
                             status_code=302, target_status_code=200)

    def test_shifts_list_view_logged_out(self):
        response = self.client.get(reverse('shiftlist'))
        self.assertRedirects(response, '/accounts/login/?next=/shifts/all/',
                             status_code=302, target_status_code=200)
...
class ShiftsModelSignalandAutoBookingTests(TestCase):
    ...
    def test_autobooking_feature(self):
        ...
        self.assertEqual(f'{self.event.start_time}',
                         f'{date.today()+timedelta(days=10)} 08:00:00+00:00')
        self.assertEqual(f'{self.event.end_time}',
                         f'{date.today()+timedelta(days=10)} 17:00:00+00:00')
